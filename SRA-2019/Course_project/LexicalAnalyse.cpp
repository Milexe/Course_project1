...
namespace Lex
{
	...
	{
		*log.stream << "" << std::endl;
		for (int i = 33; i < lextable.size; i++)
		{
	...
	{
		*log.stream << "" << std::endl;
		for (int i =9; i < idtable.size; i++)
		{
			*log.stream <<i-8<<" "<< idtable.table[i].id << " " << idtable.table[i].idxfirstLE << std::ends;
