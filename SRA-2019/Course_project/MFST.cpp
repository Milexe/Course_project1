...
bool	Mfst::start(Log::LOG& log)
{
	...
	case NS_NORULE:			MFST_TRACE4("------>NS_NORULE");
		...
		*log.stream << getDiagnosis(0, buf) << std::endl;
			std::cout << "\nGrammatic ERROR\n";
		break;
	case NS_NORULECHAIN:	MFST_TRACE4("------>NS_NORULENORULECHAIN") 	if (!rc)
		std::cout << "\nCode ERROR\n"; break;
	case NS_ERROR:			MFST_TRACE4("------>NS_ERROR")	if (!rc)
		std::cout << "\nUnknown non terminal symbol\n"; break;
	case SURPRISE:			MFST_TRACE4("------>SURPRISE") 	if (!rc)
		std::cout << "\nStep ERROR\n"; break;
	};
	return rc;
}
